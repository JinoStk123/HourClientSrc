...
public abstract class MixinEntityRenderer {
    ...
    private void updateCameraAndRender(float float1, long long2, CallbackInfo callbackInfo) {
        if (this.mc.thePlayer != null) {
            ...
            KillAura killAura = (KillAura) HourClient.moduleManager.modules.get(KillAura.class);
    ...
    private void updateRenderer(CallbackInfo callbackInfo) {
        ...
        AutoBlockIn autoBlockIn = (AutoBlockIn) HourClient.moduleManager.modules.get(AutoBlockIn.class);
    ...
    private float hurtCameraEffect(float float1) {
        if (HourClient.moduleManager == null) {
            ...
            NoHurtCam noHurtCam = (NoHurtCam) HourClient.moduleManager.modules.get(NoHurtCam.class);
    ...
    private void a(
        ...
        if (HourClient.moduleManager != null) {
            GhostHand event = (GhostHand) HourClient.moduleManager.modules.get(GhostHand.class);
            if (event.isEnabled()) {
    ...
    private double v(Vec3 vec31, Vec3 vec32) {
        if (HourClient.moduleManager == null) {
            ...
            return HourClient.moduleManager.modules.get(ViewClip.class).isEnabled() ? (double) this.thirdPersonDistance : vec31.distanceTo(vec32);
        }
    }
    ...
    private Material x(Block block) {
        if (HourClient.moduleManager == null) {
            ...
            return HourClient.moduleManager.modules.get(ViewClip.class).isEnabled() ? Material.air : block.getMaterial();
        }
    }
    ...
    private boolean y(EntityLivingBase entityLivingBase, Potion potion) {
        if (potion == Potion.blindness && HourClient.moduleManager != null) {
            AntiDebuff antiDebuff = (AntiDebuff) HourClient.moduleManager.modules.get(AntiDebuff.class);
    ...
    private boolean c(EntityPlayerSP entityPlayerSP, Potion potion) {
        if (potion == Potion.confusion && HourClient.moduleManager != null) {
            AntiDebuff antiDebuff = (AntiDebuff) HourClient.moduleManager.modules.get(AntiDebuff.class);
