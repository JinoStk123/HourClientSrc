...
public abstract class Module {
    ...
    public boolean toggle() {
        ...
        if (this.enabled == enabled) {
            if (((HUD) HourClient.moduleManager.modules.get(HUD.class)).toggleSound.getValue()) {
                HourClient.moduleManager.playSound();
            }

            // Add a transient in-game notification for toggles
            try {
                if (HourClient.notificationManager != null) {
                    ...
                    HourClient.notificationManager.add(this.getName() + " " + action, color);
                }
