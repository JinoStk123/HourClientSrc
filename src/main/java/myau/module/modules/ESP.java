...
public class ESP extends Module {
    ...
    public final ModeProperty mode = new ModeProperty("mode", 2, new String[]{"NONE", "2D", "3D", "OUTLINE", "GLOW", "FAKECORNER", "FAKE2D"});
    ...
    private Color getEntityColor(EntityPlayer entityPlayer) {
        if (TeamUtil.isFriend(entityPlayer)) {
            return myau.HourClient.friendManager.getColor();
        } else if (TeamUtil.isTarget(entityPlayer)) {
            return myau.HourClient.targetManager.getColor();
        } else {
            switch (this.color.getValue()) {
                ...
                case 2:
                    int hudColor = ((HUD) myau.HourClient.moduleManager.modules.get(HUD.class)).getColor(System.currentTimeMillis()).getRGB();
    ...
    public void onRender(Render2DEvent event) {
        if (this.isEnabled() && (this.mode.getValue() == 1 || this.mode.getValue() == 3 || this.mode.getValue() == 4 || this.healthBar.getValue() == 1)) {
            ...
            if (!renderedEntities.isEmpty()) {
                if (this.mode.getValue() == 3 || this.mode.getValue() == 4) { // OUTLINE or GLOW modes
                    ...
                    for (EntityPlayer player : renderedEntities) {
                        ...
                        // Temporarily apply glowShader color if GLOW mode, before rendering the entity
                        if (this.mode.getValue() == 4) { // GLOW
                            this.glowShader.use();
                ...
                if (this.mode.getValue() == 1 || this.healthBar.getValue() == 1) { // 2D mode or HealthBar
    ...
    public void onRender(Render3DEvent event) {
        if (this.isEnabled() && (this.mode.getValue() == 2 || this.mode.getValue() == 5 || this.mode.getValue() == 6 || this.healthBar.getValue() == 2)) {
            RenderUtil.enableRenderState();
            for (EntityPlayer player : TeamUtil.getLoadedEntitiesSorted().stream().filter(entity -> entity instanceof EntityPlayer && this.shouldRenderPlayer((EntityPlayer) entity)).map(EntityPlayer.class::cast).collect(Collectors.toList())) {
                if (player.ignoreFrustumCheck || RenderUtil.isInViewFrustum(player.getEntityBoundingBox(), 0.1F)) {
                    if (this.mode.getValue() == 2) { // 3D Box
