    ...
    public void onUpdate(UpdateEvent event) {
        if (this.isEnabled() && event.getType() == EventType.PRE) {
            ...
            if (TeamUtil.isEntityLoaded(fireball)) {
                ...
                if (!HourClient.playerStateManager.attacking && !HourClient.playerStateManager.digging && !HourClient.playerStateManager.placing) {
    ...
    public void onRender(Render3DEvent event) {
        if (this.isEnabled()) {
            if (this.showTarget.getValue() != 0 && TeamUtil.isEntityLoaded(this.target)) {
                ...
                RenderUtil.drawEntityBox(this.target, color.getRed(), color.getGreen(), color.getBlue(), 255);
