...
public class HUD extends Module {
    ...
    public void onTick(TickEvent event) {
        if (this.isEnabled() && event.getType() == EventType.POST) {
            this.activeModules = myau.HourClient.moduleManager.modules.values().stream().filter(module -> module.isEnabled() && !module.isHidden()).sorted(Comparator.comparingInt(this::getModuleWidth).reversed()).collect(Collectors.<Module>toList());
        }
    }
    ...
    public void onRender2D(Render2DEvent event) {
        ...
        if (this.chatOutline.getValue() && mc.currentScreen instanceof GuiChat) {
            ...
            if (myau.HourClient.commandManager != null && myau.HourClient.commandManager.isTypingCommand(text)) {
        ...
        if (this.isEnabled() && !mc.gameSettings.showDebugInfo) {
            ...
            if (this.blinkTimer.getValue()) {
                BlinkModules blinkingModule = myau.HourClient.blinkManager.getBlinkingModule();
                if (blinkingModule != BlinkModules.NONE && blinkingModule != BlinkModules.AUTO_BLOCK) {
                    long movementPacketSize = myau.HourClient.blinkManager.countMovement();
        ...
        try {
            if (myau.HourClient.notificationManager != null) {
                java.util.List<myau.management.NotificationManager.NotificationEntry> entries = myau.HourClient.notificationManager.getActive();
                if (!entries.isEmpty()) {
