    ...
    private Color getTargetColor(EntityLivingBase entityLivingBase) {
        if (entityLivingBase instanceof EntityPlayer) {
            if (TeamUtil.isFriend((EntityPlayer) entityLivingBase)) {
                return HourClient.friendManager.getColor();
            }
            if (TeamUtil.isTarget((EntityPlayer) entityLivingBase)) {
                return HourClient.targetManager.getColor();
            }
        }
        switch (this.color.getValue()) {
            ...
            case 1:
                int rgb = ((HUD) HourClient.moduleManager.modules.get(HUD.class)).getColor(System.currentTimeMillis()).getRGB();
...
// Add Winning/Losing label to the name using calculateWinning()
                ...
                if (this.roundedCorners.getValue()) {
                    ...
                    RenderUtil.drawRoundedRect(0, 0, (int) barTotalWidth, 27, 4, backgroundColor);
                    if (this.outline.getValue()) {
                        // For rounded outline, we draw a slightly larger rounded rect with outlineColor
                        RenderUtil.drawRoundedRect(-1, -1, (int) barTotalWidth + 2, 29, 5, outlineColor);
                    }
                    // Health bar background
                    RenderUtil.drawRoundedRect((int)(headIconOffset + 2.0F), 22, (int)(barTotalWidth - 2.0F - (headIconOffset + 2.0F)), 3, 1, ColorUtil.darker(healthBarColor, 0.2F).getRGB());
                    // Health bar foreground
                    RenderUtil.drawRoundedRect((int)(headIconOffset + 2.0F), 22, (int)(healthRatio * (barTotalWidth - 2.0F - headIconOffset - 2.0F)), 3, 1, healthBarColor.getRGB());
                } else {
