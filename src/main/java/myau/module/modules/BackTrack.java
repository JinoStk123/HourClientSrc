...
private static final Minecraft mc = Minecraft.getMinecraft();
   ...
   public final ModeProperty espMode = new ModeProperty("ESPMODE", 0, new String[]{"Hitbox", "Path", "None"});
   public final BooleanProperty renderPath;

   private final ConcurrentLinkedQueue<Vec3> path = new ConcurrentLinkedQueue<>();
   public final IntProperty maxPathLength = new IntProperty("MaxPathLength", 100, 10, 500);
   ...
   public BackTrack() {
      ...
      this.renderPath = new BooleanProperty("Render-Path", true);
   }

   @Override
   public void onEnabled() {
      Module m = myau.HourClient.moduleManager.getModule(KillAura.class);
   ...
   public void onPacket(PacketEvent event) {
      ...
      Module scaffold = myau.HourClient.moduleManager.getModule(Scaffold.class);
   ...
   public void onUpdate(UpdateEvent e) {
      ...
      if (real == null) {
         path.clear(); // Clear path if target is lost
         return;
      }
      
      // Add current real position to path
      path.add(real);
      while (path.size() > maxPathLength.getValue()) {
         path.poll();
      }
   ...
   public void onRender3D(Render3DEvent e) {
      ...
      if (espMode.getValue() == 0) { // Hitbox mode
         if (!this.renderPath.getValue()) return;
         GlStateManager.pushMatrix();
         GlStateManager.disableTexture2D();
         GlStateManager.disableDepth();
         GlStateManager.depthMask(false);
         GL11.glLineWidth(2.0f); // Set line width for path

         GlStateManager.color(0.0F, 1.0F, 1.0F, 0.8F); // Cyan color for path

         GL11.glBegin(GL11.GL_LINE_STRIP); // Draw a connected series of line segments
         ...
         for (Vec3 pos : path) {
             ...
             // Optional: If you want to draw spheres or dots at each point
             // if (prev != null) {
             //    RenderUtil.drawLine3D(prev, renderX, renderY, renderZ, 0.0F, 1.0F, 1.0F, 0.8F, 2.0F);
             // }
             // prev = new Vec3(renderX, renderY, renderZ);
         }
         GL11.glEnd();
