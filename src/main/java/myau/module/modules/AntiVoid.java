...
public class AntiVoid extends Module {
    ...
    private void resetBlink() {
        HourClient.blinkManager.setBlinkState(false, BlinkModules.ANTI_VOID);
        ...
        if (this.isEnabled()) {
            ...
            if (this.mode.getValue() == 0) {
                ...
                if (!this.wasInVoid && this.isInVoid && this.canUseAntiVoid()) {
                    HourClient.blinkManager.setBlinkState(false, BlinkModules.AUTO_BLOCK);
                    if (HourClient.blinkManager.setBlinkState(true, BlinkModules.ANTI_VOID)) {
                        this.lastSafePosition = new double[]{mc.thePlayer.prevPosX, mc.thePlayer.prevPosY, mc.thePlayer.prevPosZ};
                    }
                }
                if (HourClient.blinkManager.getBlinkingModule() == BlinkModules.ANTI_VOID
                        && this.lastSafePosition != null
                        && this.lastSafePosition[1] - (double) this.distance.getValue().floatValue() > mc.thePlayer.posY) {
                    HourClient.blinkManager
                            .blinkedPackets
                            .offerFirst(
