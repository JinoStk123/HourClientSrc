...
public class KillAura extends Module {
    ...
    public final ModeProperty targetRenderMode;
    public final ModeProperty debugLog;
    public final BooleanProperty renderTarget;
    public final BooleanProperty attackSounds = new BooleanProperty("attack-sounds", true);
    public final BooleanProperty hitSounds = new BooleanProperty("hit-sounds", true);
    ...
    private boolean performAttack(float yaw, float pitch) {
        if (!myau.HourClient.playerStateManager.digging && !myau.HourClient.playerStateManager.placing) {
            if (this.isPlayerBlocking() && this.autoBlock.getValue() != 1) {
                ...
                if ((this.rotations.getValue() != 0 || !this.isBoxInAttackRange(this.target.getBox()))
                    ...
                    if (this.attackSounds.getValue()) {
                        mc.thePlayer.playSound("myau:attack_sfx", 1.0F, 1.0F); // Generic hit sound
                    }
    ...
    private boolean canAttack() {
        if (this.inventoryCheck.getValue() && mc.currentScreen instanceof GuiContainer) {
            ...
            if (((IAccessorPlayerControllerMP) mc.playerController).getIsHittingBlock()) {
                ...
                AutoHeal autoHeal = (AutoHeal) myau.HourClient.moduleManager.modules.get(AutoHeal.class);
                if (autoHeal.isEnabled() && autoHeal.isSwitching()) {
                    ...
                    AutoBlockIn autoBlockIn = (AutoBlockIn) myau.HourClient.moduleManager.modules.get(AutoBlockIn.class);
                    if (bedNuker.isEnabled() && bedNuker.isReady()) {
                        return false;
                    } else if (myau.HourClient.moduleManager.modules.get(Scaffold.class).isEnabled()) {
                        return false;
    ...
    public KillAura() {
        ...
        this.swingRange = new FloatProperty("swing-range", 3.5F, 3.0F, 7.0F);
        this.attackRange = new FloatProperty("attack-range", 3.0F, 3.0F, 7.0F);
        this.fov = new IntProperty("fov", 360, 30, 360);
        this.minCPS = new IntProperty("min-aps", 14, 1, 30);
        this.maxCPS = new IntProperty("max-aps", 14, 1, 30);
        ...
        this.targetRenderMode = new ModeProperty("target-render-mode", 0, new String[]{"BOX", "OUTLINE", "GLOW", "RING", "LINE"});
        this.debugLog = new ModeProperty("debug-log", 0, new String[]{"NONE", "HEALTH"});
        this.renderTarget = new BooleanProperty("Render-Target", true);
    }
    ...
    public boolean isAttackAllowed() {
        Scaffold scaffold = (Scaffold) myau.HourClient.moduleManager.modules.get(Scaffold.class);
    ...
    public void onUpdate(UpdateEvent event) {
        if (event.getType() == EventType.POST && this.blinkReset) {
            ...
            myau.HourClient.blinkManager.setBlinkState(false, BlinkModules.AUTO_BLOCK);
            myau.HourClient.blinkManager.setBlinkState(true, BlinkModules.AUTO_BLOCK);
        }
                    ...
                    switch (this.autoBlock.getValue()) {
                        case 0: // NONE
                            if (PlayerUtil.isUsingItem()) {
                                ...
                                if (!this.isPlayerBlocking() && !myau.HourClient.playerStateManager.digging && !myau.HourClient.playerStateManager.placing) {
                                    swap = true;
                                }
                            } else {
                                this.isBlocking = false;
                        ...
                        case 2: // SPOOF
                            if (this.hasValidTarget()) {
                                ...
                                if (myau.HourClient.playerStateManager.digging
                                        || myau.HourClient.playerStateManager.placing
                            ...
                            if (this.hasValidTarget()) {
                                if (!myau.HourClient.playerStateManager.digging && !myau.HourClient.playerStateManager.placing) {
                                    switch (this.blockTick) {
                                        ...
                                        case 1:
                                            if (this.isPlayerBlocking()) {
                                                if(myau.HourClient.moduleManager.modules.get(NoSlow.class).isEnabled()){
                        ...
                        case 5: // INTERACT
                            if (this.hasValidTarget()) {
                                int item = ((IAccessorPlayerControllerMP) mc.playerController).getCurrentPlayerItem();
                                if (mc.thePlayer.inventory.currentItem == item && !myau.HourClient.playerStateManager.digging && !myau.HourClient.playerStateManager.placing) {
                                    switch (this.blockTick) {
                                        case 0:
                                            if (!this.isPlayerBlocking()) {
                                                swap = true;
                        ...
                        case 8: // FAKE
                            ...
                            if (PlayerUtil.isUsingItem()
                                    ...
                                    && !myau.HourClient.playerStateManager.digging
                                    && !myau.HourClient.playerStateManager.placing) {
                                swap = true;
                            }
                    }
                ...
                if (this.isBoxInSwingRange(this.target.getBox())) {
                    if (this.rotations.getValue() == 2 || this.rotations.getValue() == 3) {
                        ...
                        if (this.rotations.getValue() == 3) {
                            myau.HourClient.rotationManager.setRotation(rotations[0], rotations[1], 1, true);
                        }
        ...
        if (this.isEnabled() && !event.isCancelled() && mc.thePlayer != null && mc.theWorld != null) {
            ...
            if (this.debugLog.getValue() == 1 && this.isAttackAllowed()) {
                if (event.getPacket() instanceof S06PacketUpdateHealth) {
                    if (this.hitSounds.getValue()) {
                        float newHealth = ((S06PacketUpdateHealth) event.getPacket()).getHealth();
                        float currentHealth = mc.thePlayer.getHealth();
                        if (newHealth < currentHealth) { // Health decreased, play hurt sound
                            mc.thePlayer.playSound("myau:hit_sfx", 1.0F, 1.0F); // Generic hurt sound
                        }
                    }
    ...
    public void onRender(Render3DEvent event) {
        if (this.isEnabled() && target != null && this.renderTarget.getValue()) {
            if (this.showTarget.getValue() != 0
                ...
                switch (this.showTarget.getValue()) {
                    case 1: // DEFAULT
                        ...
                        break;
                    case 2: // HUD
                        color = ((HUD) myau.HourClient.moduleManager.modules.get(HUD.class)).getColor(System.currentTimeMillis());
                        break;
                }
                ...
                switch (this.targetRenderMode.getValue()) {
                    case 0: // BOX (filled with outline)
                        RenderUtil.drawEntityBox(this.target.getEntity(), red, green, blue, 63); // Filled box with some transparency
                        RenderUtil.drawEntityBoundingBox(this.target.getEntity(), red, green, blue, 255, 1.0f, 0.1); // Solid outline
                        break;
                    case 1: // OUTLINE
                        RenderUtil.drawEntityBoundingBox(this.target.getEntity(), red, green, blue, 255, 1.5f, 0.1); // Solid outline, slightly thicker
                        break;
                    case 2: // GLOW (filled + multiple outlines)
                        // Filled part of the glow
                        RenderUtil.drawEntityBox(this.target.getEntity(), red, green, blue, 90);

                        // Multiple outlines for glow effect
                        for (int i = 0; i < 3; i++) {
                            float expand = 0.1f + i * 0.05f; // Gradually larger outlines
                            int alpha = Math.max(0, 70 - i * 20); // Gradually less transparent
                            RenderUtil.drawEntityBoundingBox(this.target.getEntity(), red, green, blue, alpha, 1.0f, expand);
                        }
                        break;
                    case 3: // RING
                        RenderUtil.drawEntityCircle(this.target.getEntity(), this.target.getEntity().width / 1.5, 32, color.getRGB());
                        break;
                    case 4: // LINE
                        // Get player eye position
                        double playerEyeX = ((IAccessorRenderManager) mc.getRenderManager()).getRenderPosX();
                        double playerEyeY = ((IAccessorRenderManager) mc.getRenderManager()).getRenderPosY() + mc.thePlayer.getEyeHeight();
                        double playerEyeZ = ((IAccessorRenderManager) mc.getRenderManager()).getRenderPosZ();

                        // Get target entity bounding box
                        AxisAlignedBB targetBox = this.target.getEntity().getEntityBoundingBox();

                        // Adjust targetBox coordinates to render position
                        double targetMinX = targetBox.minX - ((IAccessorRenderManager) mc.getRenderManager()).getRenderPosX();
                        double targetMinY = targetBox.minY - ((IAccessorRenderManager) mc.getRenderManager()).getRenderPosY();
                        double targetMinZ = targetBox.minZ - ((IAccessorRenderManager) mc.getRenderManager()).getRenderPosZ();
                        double targetMaxX = targetBox.maxX - ((IAccessorRenderManager) mc.getRenderManager()).getRenderPosX();
                        double targetMaxY = targetBox.maxY - ((IAccessorRenderManager) mc.getRenderManager()).getRenderPosY();
                        double targetMaxZ = targetBox.maxZ - ((IAccessorRenderManager) mc.getRenderManager()).getRenderPosZ();

                        Vec3 playerEyeVec = new Vec3(playerEyeX, playerEyeY, playerEyeZ);

                        // Draw lines to corners of the target's bounding box
                        RenderUtil.drawLine3D(playerEyeVec, targetMinX, targetMinY, targetMinZ, red / 255.0f, green / 255.0f, blue / 255.0f, 1.0f, 1.0f);
                        RenderUtil.drawLine3D(playerEyeVec, targetMinX, targetMinY, targetMaxZ, red / 255.0f, green / 255.0f, blue / 255.0f, 1.0f, 1.0f);
                        RenderUtil.drawLine3D(playerEyeVec, targetMinX, targetMaxY, targetMinZ, red / 255.0f, green / 255.0f, blue / 255.0f, 1.0f, 1.0f);
                        RenderUtil.drawLine3D(playerEyeVec, targetMinX, targetMaxY, targetMaxZ, red / 255.0f, green / 255.0f, blue / 255.0f, 1.0f, 1.0f);
                        RenderUtil.drawLine3D(playerEyeVec, targetMaxX, targetMinY, targetMinZ, red / 255.0f, green / 255.0f, blue / 255.0f, 1.0f, 1.0f);
                        RenderUtil.drawLine3D(playerEyeVec, targetMaxX, targetMinY, targetMaxZ, red / 255.0f, green / 255.0f, blue / 255.0f, 1.0f, 1.0f);
                        RenderUtil.drawLine3D(playerEyeVec, targetMaxX, targetMaxY, targetMinZ, red / 255.0f, green / 255.0f, blue / 255.0f, 1.0f, 1.0f);
                        RenderUtil.drawLine3D(playerEyeVec, targetMaxX, targetMaxY, targetMaxZ, red / 255.0f, green / 255.0f, blue / 255.0f, 1.0f, 1.0f);
                        break;
                }
