...
public class PlayerStateManager {
    ...
    public void handlePacket(Packet<?> packet) {
        if (packet instanceof C02PacketUseEntity) {
            this.attacking = true;
            this.updateCombatState(); // Mark as in combat
        }
    ...
    // New method to update combat state
    private void updateCombatState() {
        this.lastCombatActionTime = System.currentTimeMillis();
        this.inCombat = true;
    }

    // New getter for inCombat
    public boolean isInCombat() {
        return this.inCombat;
    }

    // New @EventTarget method to manage combat timeout
    @EventTarget
    public void onTick(TickEvent event) {
        myau.HourClient.notificationManager.add("PlayerStateManager: Tick", 0xADD8E6); // Light blue for general tick
