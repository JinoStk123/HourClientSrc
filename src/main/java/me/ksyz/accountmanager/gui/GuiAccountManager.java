...
import net.minecraft.util.Session; // Added import
...
public class GuiAccountManager extends GuiScreen {
    ...
    private GuiTextField offlineUsernameField; // New field
    private GuiButton addOfflineButton; // New field
    ...
    public void initGui() {
        ...
        // New offline account input and button
        this.offlineUsernameField = new GuiTextField(
                6, fontRendererObj, width / 2 - 150, height - 76, 195, 20
        );
        this.offlineUsernameField.setFocused(true);
        this.offlineUsernameField.setMaxStringLength(32);
        buttonList.add(addOfflineButton = new GuiButton(
                7, width / 2 + 50, height - 76, 95, 20, "Add Offline"
        ));
    ...
    public void drawScreen(int mouseX, int mouseY, float renderPartialTicks) {
        ...
        // Render offline username field and label
        fontRendererObj.drawStringWithShadow(
            "Offline Username:", width / 2 - 150, height - 88, -1
        );
        this.offlineUsernameField.drawTextBox();
    ...
    protected void mouseClicked(int mouseX, int mouseY, int mouseButton) throws IOException {
        ...
        this.offlineUsernameField.mouseClicked(mouseX, mouseY, mouseButton); // Pass clicks to text field
    }

    @Override
    protected void keyTyped(char typedChar, int keyCode) {
        // Pass key input to the offline username field
        if (this.offlineUsernameField.isFocused()) {
            this.offlineUsernameField.textboxKeyTyped(typedChar, keyCode);
            if (keyCode == Keyboard.KEY_RETURN) { // If ENTER is pressed while focused
                actionPerformed(addOfflineButton); // Trigger the add offline account button
            }
            return; // Consume the event so it doesn't also trigger other keybinds
        }
            ...
            switch (button.id) {
                case 0: { // Login
                    if (task == null || task.isDone()) {
                        ...
                        if (account.isOffline()) { // Handle offline account login
                            String uuid = UUID.nameUUIDFromBytes(account.getUsername().getBytes(Charsets.UTF_8)).toString();
                            SessionManager.set(new Session(account.getUsername(), uuid, "0", "Legacy")); // Use SessionManager.set
                            notification = new Notification(TextFormatting.translate(String.format(
                                                "&aSuccessful offline login! (%s)&r", account.getUsername()
                                            )), 5000L);
                            return; // Exit after offline login
                        }
                ...
                break;
                case 7: { // Add Offline
                    String username = this.offlineUsernameField.getText();
                    if (!StringUtils.isBlank(username)) {
                        AccountManager.addOfflineAccount(username);
                        notification = new Notification(TextFormatting.translate(String.format(
                                                "&aAdded offline account: %s&r", username
                                            )), 5000L);
                        this.offlineUsernameField.setText(""); // Clear the field
                        AccountManager.load(); // Reload accounts to update list
                        this.guiAccountList.elementClicked(-1, false, 0, 0); // Deselect any selected account
                        this.updateScreen();
                    } else {
                        notification = new Notification(TextFormatting.translate("&cUsername cannot be empty!&r"), 5000L);
                    }
                }
                break;
                default: {
