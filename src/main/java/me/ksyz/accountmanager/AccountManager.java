...
public class AccountManager {
    ...
    private static final File file = new File(mc.mcDataDir, "hourclient.accounts.json");
    ...
    public static void addOfflineAccount(String username) {
        boolean exists = accounts.stream().anyMatch(account -> account.getUsername().equalsIgnoreCase(username));
        if (exists) {
            System.out.println("Account with username " + username + " already exists!");
            return;
        }
        accounts.add(new Account(username, true));
        save();
        System.out.println("Added offline account: " + username);
    }
            ...
            try {
                if (file.getParentFile().exists() || file.getParentFile().mkdirs()) {
                    if (file.createNewFile()) {
                        System.out.print("Successfully created hourclient.accounts.json!");
                    }
                }
            } catch (IOException e) {
                System.err.print("Couldn't create hourclient.accounts.json!");
            }
        }
    }
        ...
        try {
            ...
            if (json instanceof JsonArray) {
                ...
                for (JsonElement jsonElement : jsonArray) {
                    ...
                    boolean isOffline = Optional.ofNullable(jsonObject.get("isOffline")).map(JsonElement::getAsBoolean).orElse(false);
            ...
            System.err.print("Couldn't find hourclient.accounts.json!");
        }
    }
    ...
    public static void save() {
        try {
            ...
            for (Account account : accounts) {
                ...
                jsonObject.addProperty("isOffline", account.isOffline()); // Add this line
            ...
            System.err.print("Couldn't save hourclient.accounts.json!");
        }
    }
}
